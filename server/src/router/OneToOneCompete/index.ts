...
router.post(
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    try {
      ...
      let check1 = await prisma.user.findMany({where:{id:{in:[...user]}}})
      ...
      let result2 = await prisma.user.updateMany({where:{id:{in:[...user]}},data:{
  ...
  async (req: Request, res: Response): Promise<any> => {
    ...
    try {
      ...
      const userid = req.params.userid
        ...
        if(result1.length === 0) return res.send({success,msg:"user not exist"})
